...
#' @export
#'
sim <- function(n = 10000, trades = 390, prob = 1, spread = 0.01, volatility = 0.03, jump = 0, drift = 0, units = "day"){
  ...
  # compute prices
  p <- exp(cumsum(r)) * (1 + spread*(-0.5 + rbinom(m, size = 1, prob = 0.5)))
  ...
  for(i in 1:n){
    ...
    # OHLC
    ohlc[i,] <- c(obs[1], max(obs), min(obs), obs[length(obs)])
  }
  ...
  colnames(p) <- c("Open", "High", "Low", "Close")
